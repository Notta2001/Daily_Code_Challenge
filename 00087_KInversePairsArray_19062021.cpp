...
It's possible that some where in the right hand side the second array index become negavtive, since we cannot generate negative 
inverse pairs we just treat them as 0. but still leave the item there as a place holder.

dp[n][k] = dp[n-1][k]+dp[n-1][k-1]+...+dp[n-1][k+1-n+1]+dp[n-1][k-n+1]
dp[n][k+1] = dp[n-1][k+1]+dp[n-1][k]+...+dp[n-1][k+2-n+1]+dp[n-1][k+1-n+1]
		...
		int kInversePairs(int n , int k){
			...
			for(int i = 1; i <= n; ++i){
				...
				for(int j = 1; j <= k; ++j){
					...
					// We assume that if j < i dp[i-1][j-i] = 0 so we don't add it here
					if(j - i >= 0){
...
int main(){
	Solution obj;
	cout << obj.kInversePairs(10, 3) << endl;
}
